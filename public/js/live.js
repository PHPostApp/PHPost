const live = {
	...
	inicializar: function(){
		...
		if(live.status['nots'] == 'OFF' && live.status['mps'] == 'OFF') return true;
		// EN 30 SEGUNDOS HACE UPDATE POR AJAX
		else setTimeout(() =>  live.update(), live.update_time);
	},
			...
			if(live.focus == true){
			   ...
			   setTimeout(function(){ live.hide(); }, live.hide_time);
				...
				if(live.status['sound'] == 'ON') {
					...
					audioElement.attr({
						src: `${global_data.url}/inc/ext/new${sound_type}.mp3`,
						autoplay: 'autoplay'
					});
