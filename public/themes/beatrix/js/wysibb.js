/*
// Esto "risus" es de manera local
const urlsmiles = location.origin + '/Zero';
const smilesjson = urlsmiles + "/files/lista-smiles.json";
var lista_smiles = [];
$.getJSON(smilesjson, Smiles => {
	Smiles.map( smile => {
		const imagen = urlsmiles  + "/themes/default/images/smiles/" + smile.img;
		lista_smiles.push({
			title: smile.bbcode,
			img: `<img alt="${smile.bbcode}" src="${imagen}">`,
			bbcode: smile.bbcode
		})
	})
})
...
/**
 * Creamos un nuevo array para los nuevos colores
 * Luego recorremos todos los items y añadimos un #
 * y lo añadimos al array vacío
*/
let newColors = [];
if (typeof colores !== 'undefined') {
	colores.map( color => newColors.push(`#${color}`))
}
...
(function($) {
	...
	$.wysibb = function(txtArea,settings) {
		...
		this.options = {
			...
			buttons: "bold,italic,underline,strike,sup,sub,|,img,video,link,|,fontcolor,fontsize,fontfamily,|,smileBox,bullist,numlist,|,spoiler,messages,|,justifyleft,justifycenter,justifyright,justify,|,hr,quote,code,|,removeFormat",
			allButtons: {
				bold : {
					title: CURLANG.bold,
					buttonHTML: '<iconify-icon icon="ri:bold"></iconify-icon>',
					excmd: 'bold',
					hotkey: 'ctrl+b',
					transform : {
				...
				italic : {
					...
					buttonHTML: '<iconify-icon icon="ri:italic"></iconify-icon>',
					excmd: 'italic',
					hotkey: 'ctrl+i',
					transform : {
				...
				underline : {
					...
					buttonHTML: '<iconify-icon icon="ri:underline"></iconify-icon>',
					excmd: 'underline',
					hotkey: 'ctrl+u',
					transform : {
				...
				strike : {
					...
					buttonHTML: '<iconify-icon icon="ri:strikethrough"></iconify-icon>',
					excmd: 'strikeThrough',
					transform : {
				...
				sup : {
					title: CURLANG.sup,
					buttonHTML: '<iconify-icon icon="ri:superscript"></iconify-icon>',
					excmd: 'superscript',
					transform : {
				...
				sub : {
					title: CURLANG.sub,
					buttonHTML: '<iconify-icon icon="ri:subscript"></iconify-icon>',
					excmd: 'subscript',
					transform : {
				...
				hr : {
					title: CURLANG.hr,
					buttonHTML: '<iconify-icon icon="ri:separator"></iconify-icon>',
					transform : {
						'<hr>':"[hr]"
					}
				},
				link : {
					title: CURLANG.link,
					buttonHTML: '<iconify-icon icon="ri:attachment-2"></iconify-icon>',
					modal: {
				...
				img : {
					title: CURLANG.img,
					buttonHTML: '<iconify-icon icon="ri:image-add-fill"></iconify-icon>',
					modal: {
						...
						tabs: [
							...
							},
							{
								...
								html: '<div id="imguploader"> <form id="fupform" class="upload" action="{img_uploadurl}" method="post" enctype="multipart/form-data" target="fupload"><input type="hidden" name="iframe" value="1"/><input type="hidden" name="idarea" value="'+id+'" /><div class="fileupload"><input id="fileupl" class="file" type="file" name="img" /><button id="nicebtn" class="btn btn-primary">'+CURLANG.modal_img_btn+'</button> </div> </form> </div><iframe id="fupload" name="fupload" src="about:blank" frameborder="0" style="width:0px;height:0px;display:none"></iframe></div>'
							}
						],
				...
				bullist : {
					...
					buttonHTML: '<iconify-icon icon="ri:list-unordered"></iconify-icon>',
				...
				numlist : {
					...
					buttonHTML: '<iconify-icon icon="ri:list-ordered"></iconify-icon>',
				...
				spoiler : {
					...
					buttonHTML: '<iconify-icon icon="ri:chat-upload-line"></iconify-icon>',
					transform : {
						'<div class="spoiler"><div class="title" contenteditable="false"><a href="#" onclick="$(this).parent().find(\'.body\').hide(); return false;">Spoiler:</a></div><div class="body">{SELTEXT}</div></div>':"[spoiler]{SELTEXT}[/spoiler]",
					}
				},
				messages: {
					type: 'select',
					buttonHTML: '<iconify-icon icon="ri:chat-settings-line"></iconify-icon>',
				...
				quote : {
					title: CURLANG.quote,
					buttonHTML: '<iconify-icon icon="ri:double-quotes-r"></iconify-icon>',
					transform : {
						'<blockquote class="blockcita"><div class="cita" contenteditable="false"><strong>Cita:</strong></div><div class="citacuerpo">{SELTEXT}</div></blockquote>':"[quote]{SELTEXT}[/quote]",
						'<blockquote class="blockcita"><div class="cita" contenteditable="false"><strong>{AUTOR}</strong> dijo:</div><div class="citacuerpo">{SELTEXT}</div></blockquote>':"[quote={AUTOR}]{SELTEXT}[/quote]"
					}
				},
				code : {
					type: 'select',
					title: CURLANG.code,
					buttonHTML: '<iconify-icon icon="ri:code-s-slash-fill"></iconify-icon>',
					onlyClearText: true,
				...
				html: {
					...
					exvalue: "1",
					transform: {
						'<pre><code class="language-html">{SELTEXT}</code></pre>':'[html]{SELTEXT}[/html]'
					}
				},
				css: {
					title: "CSS",
					...
					exvalue: "2",
					transform: {
						'<pre><code class="language-css">{SELTEXT}</code></pre>':'[css]{SELTEXT}[/css]'
					}
				},
				javascript: {
					title: "JavaScript",
					...
					exvalue: "3",
					transform: {
						'<pre><code class="language-javascript">{SELTEXT}</code></pre>':'[javascript]{SELTEXT}[/javascript]'
					}
				},
				php: {
					title: "PHP",
					...
					exvalue: "4",
					transform: {
						'<pre><code class="language-php">{SELTEXT}</code></pre>':'[php]{SELTEXT}[/php]'
					}
				},
				sql: {
					title: "SQL",
					...
					exvalue: "4",
					transform: {
						'<pre><code class="language-sql">{SELTEXT}</code></pre>':'[sql]{SELTEXT}[/sql]'
					}
				},
				fontcolor: {
					type: "colorpicker",
				...
				fontsize: {
					...
					buttonHTML: '<iconify-icon icon="ri:font-size"></iconify-icon>',
					options: "fs_verysmall,fs_small,fs_normal,fs_big,fs_verybig"
				},
				fontfamily: {
					type: 'select',
					title: CURLANG.fontfamily,
					buttonHTML: '<iconify-icon icon="ri:font-family"></iconify-icon>',
					excmd: 'fontName',
					valueBBname: "font",
					options: [
				...
				smilebox: {
					...
					buttonHTML: '<iconify-icon icon="ri:user-smile-line"></iconify-icon>'
				},
				justify: {
					title: CURLANG.justify,
					buttonHTML: '<iconify-icon icon="ri:align-justify"></iconify-icon>',
					groupkey: 'align',
					transform: {
				...
				justifyleft: {
					...
					buttonHTML: '<iconify-icon icon="ri:align-left"></iconify-icon>',
					groupkey: 'align',
					transform: {
				...
				justifyright: {
					...
					buttonHTML: '<iconify-icon icon="ri:align-right"></iconify-icon>',
					groupkey: 'align',
					transform: {
				...
				justifycenter: {
					...
					buttonHTML: '<iconify-icon icon="ri:align-center"></iconify-icon>',
					groupkey: 'align',
					transform: {
				...
				video: {
					title: CURLANG.video,
					buttonHTML: '<iconify-icon icon="ri:video-line"></iconify-icon>',
					modal: {
						title: CURLANG.video,
						width: "600px",
						tabs: [
					...
					transform: {
						'<lite-youtube videoid="{SRC}" style="background-image: url(\'https://i.ytimg.com/vi/{SRC}/maxresdefault.jpg\');" params="controls=0&start=10&end=30&modestbranding=2&rel=0&enablejsapi=1"></lite-youtube>':'[swf=http://www.youtube.com/v/{SRC}]'
					}
				},
				//select options
				fs_verysmall: {
				...
				removeformat: {
					...
					buttonHTML: '<iconify-icon icon="ri:format-clear"></iconify-icon>',
					excmd: "removeFormat"
				}
			},
			systr: {
			...
			smileList: [
				// SMILES DEFAULT
				{title: "xd", img: '<img src="'+global_data.public_images+'/smiles/041.png">', bbcode:"xd"},
				{title: ":)", img: '<img src="'+global_data.public_images+'/smiles/001.png">', bbcode:":)"},
				{title: ":D", img: '<img src="'+global_data.public_images+'/smiles/002.png">', bbcode:":D"},
				{title: ";)", img: '<img src="'+global_data.public_images+'/smiles/003.gif">', bbcode:";)"},
				{title: ":O", img: '<img src="'+global_data.public_images+'/smiles/004.png">', bbcode:":O"},
				{title: "(H)", img: '<img src="'+global_data.public_images+'/smiles/006.png">', bbcode:"(H)"},
				{title: ":P", img: '<img src="'+global_data.public_images+'/smiles/104.png">', bbcode:":P"},
				{title: "8o|", img: '<img src="'+global_data.public_images+'/smiles/049.png">', bbcode:"8o|"},
				{title: ":S", img: '<img src="'+global_data.public_images+'/smiles/009.png">', bbcode:":S"},
				{title: ":$", img: '<img src="'+global_data.public_images+'/smiles/008.png">', bbcode:"_$"},
				{title: ":(", img: '<img src="'+global_data.public_images+'/smiles/010.png">', bbcode:":("},
				{title: ":'(", img: '<img src="'+global_data.public_images+'/smiles/011.gif">', bbcode:":'("},
				{title: ":|", img: '<img src="'+global_data.public_images+'/smiles/012.png">', bbcode:":|"},
				{title: "(6)", img: '<img src="'+global_data.public_images+'/smiles/013.png">', bbcode:"(6)"},
				{title: "8-|", img: '<img src="'+global_data.public_images+'/smiles/050.png">', bbcode:"8-|"},
				{title: ":-/", img: '<img src="'+global_data.public_images+'/smiles/083.png">', bbcode:":-/"},
				{title: "^o)", img: '<img src="'+global_data.public_images+'/smiles/051.png">', bbcode:"^o)"},
				{title: "(A)", img: '<img src="'+global_data.public_images+'/smiles/014.png">', bbcode:"(A)"},
				{title: ":[", img: '<img src="'+global_data.public_images+'/smiles/043.png">', bbcode:":["},
				{title: ":-#", img: '<img src="'+global_data.public_images+'/smiles/048.png">', bbcode:":-#"},
				{title: ":-*", img: '<img src="'+global_data.public_images+'/smiles/052.png">', bbcode:":-*"},
				{title: "+o(", img: '<img src="'+global_data.public_images+'/smiles/053.png">', bbcode:"+o("},
				{title: "(brb)", img: '<img src="'+global_data.public_images+'/smiles/066.gif">', bbcode:"(brb)"},
				{title: ":^)", img: '<img src="'+global_data.public_images+'/smiles/072.gif">', bbcode:":^)"},
				{title: "*-)", img: '<img src="'+global_data.public_images+'/smiles/073.gif">', bbcode:"*-)"},
				{title: "<o)", img: '<img src="'+global_data.public_images+'/smiles/075.gif">', bbcode:"<o)"},
				{title: "8-)", img: '<img src="'+global_data.public_images+'/smiles/076.gif">', bbcode:"|-)"},
				{title: "|-)", img: '<img src="'+global_data.public_images+'/smiles/078.gif">', bbcode:";-/"},
				{title: ";-/", img: '<img src="'+global_data.public_images+'/smiles/082.png">', bbcode:"(jk)"},
				{title: "(jk)", img: '<img src="'+global_data.public_images+'/smiles/084.png">', bbcode:"(j)"},
				{title: "(j)", img: '<img src="'+global_data.public_images+'/smiles/086.png">', bbcode:"(V)"},
				{title: "(V)", img: '<img src="'+global_data.public_images+'/smiles/087.png">', bbcode:"(lol)"},
				{title: "(lol)", img: '<img src="'+global_data.public_images+'/smiles/089.gif">', bbcode:"(xD)"},
				{title: "(xD)", img: '<img src="'+global_data.public_images+'/smiles/090.png">', bbcode:"(xD)"},
				{title: ":8)", img: '<img src="'+global_data.public_images+'/smiles/088.png">', bbcode:":8"},
				{title: "(ff)", img: '<img src="'+global_data.public_images+'/smiles/091.gif">', bbcode:"(ff)"},
				{title: "(fm)", img: '<img src="'+global_data.public_images+'/smiles/092.gif">', bbcode:"(fm)"},
				{title: ":'|", img: '<img src="'+global_data.public_images+'/smiles/093.gif">', bbcode:":'|"},
				{title: ":]", img: '<img src="'+global_data.public_images+'/smiles/094.gif">', bbcode:":]"},
				{title: ":}", img: '<img src="'+global_data.public_images+'/smiles/095.png">', bbcode:":}"},
				{title: "(BOO)", img: '<img src="'+global_data.public_images+'/smiles/096.png">', bbcode:"(BOO)"},
				{title: "*|", img: '<img src="'+global_data.public_images+'/smiles/097.gif">', bbcode:"*|"},
				{title: "*\\", img: '<img src="'+global_data.public_images+'/smiles/098.png">', bbcode:"*\\"},
				{title: "(wm)", img: '<img src="'+global_data.public_images+'/smiles/100.png">', bbcode:"(wm)"},
				{title: "(xo)", img: '<img src="'+global_data.public_images+'/smiles/101.gif">', bbcode:"(xo)"},
				{title: "(l)", img: '<img src="'+global_data.public_images+'/smiles/015.png">', bbcode:"(l)"},
				{title: "(u)", img: '<img src="'+global_data.public_images+'/smiles/016.png">', bbcode:"(u)"},
				{title: "(@)", img: '<img src="'+global_data.public_images+'/smiles/018.png">', bbcode:"(@)"},
				{title: "(&)", img: '<img src="'+global_data.public_images+'/smiles/019.png">', bbcode:"(&)"},
				{title: "(S)", img: '<img src="'+global_data.public_images+'/smiles/020.png">', bbcode:"(S)"},
				{title: "(*)", img: '<img src="'+global_data.public_images+'/smiles/021.png">', bbcode:"(*)"},
				{title: "(~)", img: '<img src="'+global_data.public_images+'/smiles/022.png">', bbcode:"(~)"},
				{title: "(8)", img: '<img src="'+global_data.public_images+'/smiles/023.png">', bbcode:"(8)"},
				{title: "(E)", img: '<img src="'+global_data.public_images+'/smiles/024.png">', bbcode:"(E)"},
				{title: "(F)", img: '<img src="'+global_data.public_images+'/smiles/025.png">', bbcode:"(F)"},
				{title: "(W)", img: '<img src="'+global_data.public_images+'/smiles/026.png">', bbcode:"(W)"},
				{title: "(O)", img: '<img src="'+global_data.public_images+'/smiles/027.gif">', bbcode:"(O)"},
				{title: "(K)", img: '<img src="'+global_data.public_images+'/smiles/028.png">', bbcode:"(K)"},
				{title: "(G)", img: '<img src="'+global_data.public_images+'/smiles/029.png">', bbcode:"(G)"},
				{title: "(^)", img: '<img src="'+global_data.public_images+'/smiles/030.png">', bbcode:"(^)"},
				{title: "(P)", img: '<img src="'+global_data.public_images+'/smiles/031.png">', bbcode:"(P)"},
				{title: "(I)", img: '<img src="'+global_data.public_images+'/smiles/032.png">', bbcode:"(I)"},
				{title: "(C)", img: '<img src="'+global_data.public_images+'/smiles/033.png">', bbcode:"(C)"},
				{title: "(T)", img: '<img src="'+global_data.public_images+'/smiles/034.png">', bbcode:"(T)"},
				{title: "({)", img: '<img src="'+global_data.public_images+'/smiles/035.png">', bbcode:"({)"},
				{title: "(})", img: '<img src="'+global_data.public_images+'/smiles/036.png">', bbcode:"(})"},
				{title: "(B)", img: '<img src="'+global_data.public_images+'/smiles/037.png">', bbcode:"(B)"},
				{title: "(D)", img: '<img src="'+global_data.public_images+'/smiles/038.png">', bbcode:"(D)"},
				{title: "(Z)", img: '<img src="'+global_data.public_images+'/smiles/039.png">', bbcode:"(Z)"},
				{title: "(X)", img: '<img src="'+global_data.public_images+'/smiles/040.png">', bbcode:"(X)"},
				{title: "(Y)", img: '<img src="'+global_data.public_images+'/smiles/041.png">', bbcode:"(Y)"},
				{title: "(N)", img: '<img src="'+global_data.public_images+'/smiles/042.png">', bbcode:"(N)"},
				{title: "(nnh)", img: '<img src="'+global_data.public_images+'/smiles/044.png">', bbcode:"(nnh)"},
				{title: "(#)", img: '<img src="'+global_data.public_images+'/smiles/046.png">', bbcode:"(#)"},
				{title: "(R)", img: '<img src="'+global_data.public_images+'/smiles/047.png">', bbcode:"(R)"},
				{title: "(sn)", img: '<img src="'+global_data.public_images+'/smiles/054.png">', bbcode:"(sn)"},
				{title: "(tu)", img: '<img src="'+global_data.public_images+'/smiles/055.png">', bbcode:"(tu)"},
				{title: "(pl)", img: '<img src="'+global_data.public_images+'/smiles/056.png">', bbcode:"(pl)"},
				{title: "(||)", img: '<img src="'+global_data.public_images+'/smiles/057.png">', bbcode:"(||)"},
				{title: "(pi)", img: '<img src="'+global_data.public_images+'/smiles/058.png">', bbcode:"(pi)"},
				{title: "(so)", img: '<img src="'+global_data.public_images+'/smiles/059.png">', bbcode:"(so)"},
				{title: "(au)", img: '<img src="'+global_data.public_images+'/smiles/060.png">', bbcode:"(au)"},
				{title: "(ap)", img: '<img src="'+global_data.public_images+'/smiles/061.png">', bbcode:"(ap)"},
				{title: "(um)", img: '<img src="'+global_data.public_images+'/smiles/062.png">', bbcode:"(um)"},
				{title: "(ip)", img: '<img src="'+global_data.public_images+'/smiles/063.png">', bbcode:"(ip)"},
				{title: "(co)", img: '<img src="'+global_data.public_images+'/smiles/064.png">', bbcode:"(co)"},
				{title: "(mp)", img: '<img src="'+global_data.public_images+'/smiles/065.png">', bbcode:"(mp)"},
				{title: "(st)", img: '<img src="'+global_data.public_images+'/smiles/067.png">', bbcode:"(st)"},
				{title: "(pu)", img: '<img src="'+global_data.public_images+'/smiles/102.png">', bbcode:"(pu)"},
				{title: "(yn)", img: '<img src="'+global_data.public_images+'/smiles/068.png">', bbcode:"(yn"},
				{title: "(h5)", img: '<img src="'+global_data.public_images+'/smiles/069.gif">', bbcode:"(h5)"},
				{title: "(mo)", img: '<img src="'+global_data.public_images+'/smiles/070.png">', bbcode:"(mo)"},
				{title: "(bah)", img: '<img src="'+global_data.public_images+'/smiles/071.png">', bbcode:"(bah)"},
				{title: "(li)", img: '<img src="'+global_data.public_images+'/smiles/074.gif">', bbcode:"(li)"},
				{title: "(wo)", img: '<img src="'+global_data.public_images+'/smiles/077.png">', bbcode:"(wo)"},
				{title: "'.'", img: '<img src="'+global_data.public_images+'/smiles/080.png">', bbcode:"'.'"},
				{title: "(bus)", img: '<img src="'+global_data.public_images+'/smiles/045.png">', bbcode:"(bus)"},
				{title: "*p*", img: '<img src="'+global_data.public_images+'/smiles/079.png">', bbcode:"*p*"},
				{title: "*s*", img: '<img src="'+global_data.public_images+'/smiles/085.png">', bbcode:"*s*"},
				{title: "(M)", img: '<img src="'+global_data.public_images+'/smiles/017.png">', bbcode:"(M)"},
				{title: "(xx)", img: '<img src="'+global_data.public_images+'/smiles/103.png">', bbcode:"(xx)"},
			],
	...
	$.wysibb.prototype = {
		...
		build: function() {
			...
			const myTheme = document.querySelector('html').getAttribute('data-theme')
			//this.$editor = $('<div class="wysibb">');
			this.$editor = $('<div>').addClass("wysibb theme-" + myTheme);
			...
			this.$editor.append('<span class="powered">Powered by <a class="fw-bolder" href="https://phpost.es" rel="noreferrer" target="_blank">PHPost.es</a></span>');
		...
		buildColorpicker: function(container,bn,opt) {
			var $btn = $('<div class="wysibb-toolbar-btn wbb-dropdown wbb-cp">').appendTo(container).append('<div class="ve-tlb-colorpick"><iconify-icon icon="ri:font-color"></iconify-icon></div>').append(this.strf('<span class="btn-tooltip">{title}<ins/></span>',{title:opt.title}));
			var $cpline = $btn.find(".cp-line");
			
			var $dropblock = $('<div class="wbb-list wbb-color-picker">').appendTo($btn); 
		...
		buildSelect: function(container,bn,opt) {
			...
			var $btn = $('<div class="wysibb-toolbar-btn wbb-select wbb-'+bn+'">').appendTo(container).append($btnHTML).append(this.strf('<span class="btn-tooltip">{title}<ins/></span>',{title:opt.title}));  
		...
		initModal: function() {
			...
			if (this.$modal.length==0) {
				...
				.html(`<div class="wbbm wbb-modal">
						<div class="wbbm-title wbb-modal-title">
							<span class="wbbm-title-text wbb-modal-title-text"></span>
							<span class="wbbclose" title="${CURLANG.close}">×</span>
						</div>
						<div class="wbbm-content wbb-modal-content"></div>
						<div class="wbbm-bottom wbb-modal-bottom">
							<button id="wbbm-submit" class="btn btn-success">${CURLANG.save}</button>
							<button id="wbbm-cancel" class="btn btn-warning">${CURLANG.cancel}</button>
							<button id="wbbm-remove" class="btn btn-danger">${CURLANG.remove}</button>
						</div>
					</div>`).hide();
				...
				if (r.html) {
					...
					$.each(r.input,$.proxy(function(j,inp) {
						...
						if (inp.type && inp.type=="div") {
							...
							$c.append(this.strf('<div class="wbbm-inp-row mb-3"><label>{title}</label><div class="inp-text div-modal-text form-control" contenteditable="true" name="{param}">{value}</div></div>',inp));
						}else{
							//default input
							$c.append(this.strf('<div class="wbbm-inp-row"><label>{title}</label><input class="inp-text form-control modal-text" type="text" name="{param}" value="{value}"/></div>',inp));
						}
