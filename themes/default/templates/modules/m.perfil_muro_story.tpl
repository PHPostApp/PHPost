{foreach from=$tsMuro.data item=p}
	<div class="Story" id="pub_{$p.pub_id}">
		<a href="{$tsConfig.url}/perfil/{$p.user_name}" class="Story_Pic">
			<img alt="{$p.user_name}" class="image" src="{$tsConfig.images}/mantenimiento.gif" data-src="{$tsConfig.avatar}/{$p.p_user_pub}_50.jpg?{$smarty.now}"/>
		</a>
			...
			<div class="Story_Head">
				{if $p.p_user == $tsUser->uid || $p.p_user_pub == $tsUser->uid || $tsUser->is_admod || $tsUser->permisos.moepm}
					...
					<div class="Story_Hide">
						<a href="#" onclick="muro.del_pub({$p.pub_id},1); return false;" title="Eliminar la publicaci&oacute;n" class="uiClose"></a>
					</div>
				{/if}
				<div class="Story_Message">
					<div class="autor">
						<a href="{$tsConfig.url}/perfil/{$p.user_name}" class="a_blue">{if $p.user_name == $tsUser->nick}{$tsUser->nick}{else}{$p.user_name}{/if}</a>
					</div>
					<span>{$p.p_body|quot}</span>
					{if $p.p_type != 1}
						<div class="mvm clearfix">
							{if $p.p_type == 2}
								<a href="javascript:muro.load_atta('foto', '{$p.a_url}', this)" class="uiPhoto"><img src="{$p.a_img}"/></a>
							{elseif $p.p_type == 3}
							  	<div class="uiLink">
							...
							{elseif $p.p_type == 4}
							 	<lite-youtube videoid="{$p.a_url}" style="background-image: url('https://i.ytimg.com/vi/{$p.a_url}/maxresdefault.jpg');"></lite-youtube>
							  	<div class="videoDesc">
