		...
		<ul>
			{if $tsPosts}
				{foreach from=$tsPosts item=p}
				 	<article itemscope itemtype="https://schema.org/Article" class="categoriaPost" style="background-image:url({$tsConfig.assets}/images/icons/cat/{$p.c_img}); {if $p.post_status == 3} background-color:#f1f1f1; {elseif $p.post_status == 1}background-color:coral;{elseif $p.post_status == 2} background-color:rosyBrown;{elseif $p.user_activo == 0} background-color:burlyWood;{elseif $p.user_baneado == 1} background-color:orange;{/if}" >
