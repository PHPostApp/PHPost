<div class="subMenuContent">
	{if $tsPage != 'tops'}
	<div id="subMenuPosts" class="subMenu">
		<ul class="tabsMenu">
		  	...
		  	{if $tsUser->is_member}
		  		{if $tsUser->is_admod || $tsUser->permisos.gopp}
		  			<li{if $tsSubmenu == 'agregar'} class="here"{/if}><a class=vctip title="Agregar Post" href="{$tsConfig.url}/agregar/">Agregar Post</a></li>
		  		{/if}
		...
		<div id="subMenuFotos" class="subMenu">
			<ul class="tabsMenu">
				...
				{if $tsAction == 'album' && $tsFUser.0 != $tsUser->uid}
					<li class="here"><a href="{$tsConfig.url}/fotos/{$tsFUser.1}">&Aacute;lbum de {$tsFUser.1}</a></li>
				{/if}
