...
(function ($) {
	$.fn.tooltip = function (opts) {
		...
		var mouseOutCallback = function (obj) {
			...
			if (tooltip && !$.data(tooltip[0], 'hover')) tooltip.hide();
		}
		...
		this.on('mouseenter', function () {
			...
			setTimeout(function () {
				...
				if (!$.data(obj, 'tooltip')) {
					...
					const vcard = `<div class="bubble-wrapper">
					...
					tooltip.hover(() => $.data(this, 'hover', true), () => {
						$.removeData(this, 'hover');
						setTimeout(() => mouseOutCallback(obj), opts.outDelay);
					});
				} else {
				...
				// TOP
				if(move_more) tooltip.css({top: pos.top + opts.offset[0]});
			}, opts.inDelay);
		}).on('mouseleave', () => {
			var obj = this;
			setTimeout(() => mouseOutCallback(obj), opts.outDelay);
