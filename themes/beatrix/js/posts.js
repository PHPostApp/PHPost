...
insertarRed = () => {
	$('.post-meta-social > a[role=button]').map( (i, red) => {
		let socialred = '#' + red.id;
		if(socialred === '#web') {
			$(socialred).attr('href', `javascript:notifica.sharePost(${global_data.postid})`)
		} else {
			$(socialred).attr('href', `javascript:compartir('${socialred.replace('#', '')}', '${$(socialred).data('title')}')`)
		}
	})
}
