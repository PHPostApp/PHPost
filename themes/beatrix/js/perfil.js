...
const perfilJS = (() => {
	...
	function editarPresentacion(gew = false) {
		var pid = $('#info').attr('pid');
		if(!gew) {
			$.post(global_data.url + '/perfil-editar-presentacion.php', { pid },  mensaje => {
				mydialog.faster({
					title: 'Editar presentaci&oacute;n',
					body: '<textarea maxlength="60" name="mensaje" id="mensaje" placeholder="Mensaje Personal" class="form-control">'+mensaje+'</textarea>',
					buttons: {
						ok: { text: 'Guardar cambios', action: `perfilJS.editarmsg(true)` },
						fail: { text: 'Cancelar', action: 'close' }
					}
				})
			})
		} else {
			var data = 'save=' + true + '&mensaje='+ $('textarea[name=mensaje]').val() + '&pid=' + pid;
			$.post(global_data.url + '/perfil-editar-presentacion.php', data,  response => {
				if(response) {
					$('.frase-personal').html($('textarea[name=mensaje]').val())
					mydialog.close();
				}
			})
		}
	...
	function mostrar_opciones() {
		$(".dropdown-menu")[(abierto ? 'addClass' : 'removeClass')]('show');
		abierto = abierto ? false : true;
	}

	// Hace pública la función
	return {
    	editarmsg: editarPresentacion,
...
var muro = {
		  ...
		  validar: function(inpt){
				...
				if(val == '' || val == inpt.attr('title') || regex.test(val) == false) return 'Debes ingresar una direcci&oacute;n URL v&aacute;lida.';
					 ...
					 switch(muro.stream.type){
						  ...
						  break;
						  case 'video':
								// https://youtu.be/XsIsqOmeMHc?list=RD24gQ4vxrN5g
	 ...
	 load_atta: function(type, ID, obj){
		  switch(type){
				...
				break;
				case 'video':
					 var content = `<lite-youtube videoid="${ID}" style="background-image: url('https://i.ytimg.com/vi/${ID}/maxresdefault.jpg');" params="controls=0&start=10&end=30&modestbranding=2&rel=0&enablejsapi=1"></lite-youtube>`;
				break;
		  }
		  // CARGAMOS
...
let mycover = $('#portadaurl').data('url');
if(!empty(mycover)) {
	$(".background-profile").css({
		backgroundImage: 'url("'+mycover+'")'
	}).addClass('image');
}
