<div class="post-contenedor">
	<div class="post-title">
      ...
      {if $tsPost.puntos && ($tsPost.post_user == $tsUser->uid || $tsUser->is_admod)}
         ...
         <img style="margin-top: 5px; margin-bottom: -21px; cursor:pointer;" title="Puntos entregados" onclick="$('#ver_puntos').slideToggle(); return false" src="{$tsConfig.images}/arrowdown.png"/>
      {/if}
   </div>
   <div class="post-contenido">
      {if !$tsUser->is_member}{include "m.global_ads_728.tpl"}{/if}
         {if $tsPost.post_user == $tsUser->uid && $tsUser->is_admod == 0 && $tsUser->permisos.most == false && $tsUser->permisos.moayca == false && $tsUser->permisos.moo == false && $tsUser->permisos.moep ==  false && $tsUser->permisos.moedpo == false}
				<div class="floatR">
               <a title="Borrar Post" onclick="borrar_post(); return false;" href="" class="btnActions"><img alt="Borrar" src="{$tsConfig.images}/borrar.png"/> Borrar</a>
               <a title="Editar Post" href="{$tsConfig.url}/posts/editar/{$tsPost.post_id}" class="btnActions"><img alt="Editar" src="{$tsConfig.images}/editar.png"/> Editar</a>
            </div>
      ...
      {if $tsPost.user_firma && $tsConfig.c_allow_firma}
         <hr class="divider" />
         <span>{$tsPost.user_firma}</span>
      {/if}
      <div class="compartir-mov" style="text-align: right; color:#888; font-size: 14px;margin: 30px 0 10px">
         <div class="m-left"></div>
         <div class="m-right"></div>
         <ul class="post-compartir clearbeta" data-url="{$tsConfig.url}/posts/{$tsPost.categoria.c_seo}/{$tsPost.post_id}/{$tsPost.post_title|seo}.html" data-title="{$tsPost.post_title}">
            <li class="share-big red" id="whatsapp"></li>
            <li class="share-big red" id="telegram"></li>
            <li class="share-big red" id="twitter"></li>
            <li class="share-big red" id="facebook"></li>
            <li class="share-big">
