...
<div class="sidebar">
	<div class="section-title">
		<h2 class="title-block">Detalles</h2>
	</div>
	<div class="">
		<span class="frase-personal text-center d-block px-3 py-2">{$tsInfo.p_mensaje}</span>
		{if $tsUser->is_member && $tsUser->uid == $tsInfo.uid}
			<a href="javascript:perfilJS.editarmsg()" class="btn btn-sm btn-primary d-block my-2">Editar presentación</a>
		{/if}
		{if !empty($tsInfo.user_pais)}<span class="d-block my-3"><iconify-icon icon="et:map"></iconify-icon> Vive en {$tsInfo.user_pais}</span>{/if}
		<div class="sitios">
		{if !empty($tsInfo.p_socials)}
		      ...
		      {foreach $tsRedes key=name item=red}
		         {if $tsInfo.p_socials.$name != ''}
		            {assign var="redConContenido" value="<a class='sitio icon' target='_blank' href='{$red.url}/{$tsInfo.p_socials.$name}' title='{$red.nombre}'><iconify-icon icon='{$red.iconify}'></iconify-icon></a>"}
		            {append var="redesConContenido" value=$redConContenido}
		         {else}
		            {assign var="redSinContenido" value="<span class='sitio icon icon_off'><iconify-icon icon='{$red.iconify}'></iconify-icon></span>"}
...
{if $tsGeneral.fotos_total > 0}
	<div class="sidebar">
		<div class="section-title">
			<h2 class="title-block">Fotos</h2>
			<a href="{$tsConfig.url}/fotos/{$tsInfo.nick}" class="subtitle-block">Ver todas las fotos</a>
		</div>
