...
\*********************************/
	...
	$files = array(
		...
		'perfil-editar-presentacion' => array('n' => 2, 'p' => ''),
	 );
...
\*********************************/
	...
	switch($action){
		...
		case 'perfil-siguiendo':
			...
			$smarty->assign("tsData", $tsMonitor->getFollows($type, $user_id));
			//--->
		break;
		...
		case 'perfil-comunidades':
			...
			//--->
		break;
		case 'perfil-editar-presentacion':
			$save = isset($_POST['save']) ? $tsCore->setSecure($_POST['save']) : '';
			echo $tsCuenta->Message($user_id, $save);
		break;
		default:
