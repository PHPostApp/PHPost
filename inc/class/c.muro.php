	...
	public function ajaxCheck($return = false, $urlin = null){
		...
		switch($type){
			...
			case 'foto':
				...
				elseif($data[0] < $min || $data[1] < $min) return "0: Tu foto debe tener un tama&ntilde;o superior a {$min}x{$min} pixeles.";
				elseif($data[0] > $max || $data[1] > $max) return "0: Tu foto debe tener un tama&ntilde;o menor a {$max}x{$max} pixeles.";
				else {
					return ($return == false) ? '1: <center><img src="'.$tsCore->setSecure($url, true).'"/></center>' : $tsCore->setSecure($url, true);
				}
			break;
			// VALIDAR URL
			case 'enlace':
				...
				case 'video':
					// Nuevo patron para validar la url de youtube (https://youtu.be/__ID__ o ttps://www.youtube.com/watch?v=__ID__)
