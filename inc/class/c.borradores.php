...
class tsDrafts {
	...
	*/
	public function getDrafts(){
		global $tsCore, $tsUser;
		//
		$drafts = result_array(db_exec([__FILE__, __LINE__], 'query', "SELECT c.c_nombre, c.c_seo, c.c_img, b.bid, b.b_title, b.b_date, b.b_status, b.b_causa FROM p_categorias AS c LEFT JOIN p_borradores AS b ON c.cid = b.b_category WHERE b.b_user = {$tsUser->info['user_id']} ORDER BY b.b_date"));
		// SET
	...
	*/
	public function getDraft(int $status = 1){
		...
		//
		$bid = (int)$_GET['action'];
      return db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', "SELECT bid, b_user, b_date, b_title, b_body, b_tags, b_category, b_private, b_block_comments, b_sponsored, b_sticky, b_smileys, b_post_id, b_status, b_causa FROM `p_borradores` WHERE `bid` = $bid AND `b_user` = {$tsUser->info['user_id']} AND b_status = $status LIMIT 1"));
	}
	/*
		delDraft()
	*/
	public function delDraft(){
		...
		//
		$bid = (int)$_POST['borrador_id'];
      if(db_exec([__FILE__, __LINE__], 'query', "DELETE FROM `p_borradores` WHERE `bid` = $bid AND `b_user` = {$tsUser->info['user_id']}")) return '1: Borrador eliminado';
