...
class tsAfiliado {
	...
	public function getAfiliados(string $type = 'home'){
		...
		//
		return result_array(db_exec([__FILE__, __LINE__], 'query', "SELECT $select FROM w_afiliados $from"));
	}
	public function getAfiliado(string $type = 'home'){
		$aid = ($type === 'home') ? (int)$_POST['ref'] : (int)$_GET['aid'];
		$data = db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', "SELECT aid, a_titulo, a_url, a_banner, a_descripcion FROM w_afiliados WHERE aid = $aid"));
		return $data;
	}
	public function newAfiliado(){
		...
		//
		if(insertInto([__FILE__, __LINE__], 'w_afiliados', $dataIn, 'a_')) {
	...
	public function EditarAfiliado() {
		...
		return (db_exec([__FILE__, __LINE__], 'query', "UPDATE w_afiliados SET $afs WHERE aid = $afiliado")) ? '1: Guardado' : '0: Ocurri&oacute; un error';
	}
	public function DeleteAfiliado(int $aid = 0){
		global $tsUser;
		if($tsUser->is_admod == 1) {
			if(deleteID([__FILE__, __LINE__], 'w_afiliados', "aid = $aid")) return '1: Afiliado eliminado';
	...
	public function SetActionAfiliado(){
		...
		$active = $status ? 0 : 1;
		if(db_exec([__FILE__, __LINE__], 'query', "UPDATE w_afiliados SET a_active = $active WHERE aid = $afiliado")) return ($status ? 2 : 1) . ': Afiliado ' . ($status ? 'des' : '') . 'habilitado';
	...
	public function urlOut(){
		global $tsCore;
		  //
		$ref = (int)$_GET['ref'];
		$data = db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', "SELECT a_url, a_sid FROM w_afiliados WHERE aid = $ref LIMIT 1"));
		if(isset($data['a_url'])) {
			db_exec([__FILE__, __LINE__], 'query', "UPDATE `w_afiliados` SET a_hits_out = a_hits_out + 1 WHERE aid = $ref");
			// Y REDIRECCIONAMOS
	...
	public function urlIn(){
		...
		$ref = (int)$_GET['ref'];
		if($ref > 0) {
			db_exec([__FILE__, __LINE__], 'query', "UPDATE `w_afiliados` SET a_hits_in = a_hits_in + 1 WHERE aid = $ref");
		}
