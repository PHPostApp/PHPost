...
class UpdateGithub {
	...
	*/
	...
	public function updateTable(bool $action = true) {
		global $tsCore;
		// Esta actualizado?
		$status = db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', "SELECT updated FROM w_configuracion WHERE tscript_id = 1"));
		if((int)$status['updated'] === 0 OR $action === false) {
			$change = $action ? 1 : 0;
			return (db_exec([__FILE__, __LINE__], 'query', "UPDATE w_configuracion SET updated = $change WHERE tscript_id = 1")) ? 'La tabla sea actualizado correctamente' : 'No se pudo actualizar';
