...
class tsActividad {
	...
	public function setActividad($ac_type = NULL, $obj_uno = NULL, $obj_dos = 0){
		...
		# INSERCION DE DATOS        
		return (db_exec([__FILE__, __LINE__], 'query', "INSERT INTO `u_actividad` (`user_id`, `obj_uno`, `obj_dos`, `ac_type`, `ac_date`) VALUES ({$tsUser->uid}, $obj_uno, $obj_dos, $ac_type, $ac_date)"));
	}
	/**
	 * @name getActividad
	 * @access public
	...
	private function makeOracion($data = NULL){
		...
		switch($ac_type){
			...
			case 8:
				// AVATARES
				$img_uno = '<img src="'.$tsCore->settings['url'].'/files/avatar/'.$data['user_id'].'_50.jpg"/>';
				$img_dos = '<img src="'.$tsCore->settings['url'].'/files/avatar/'.$data['avatar'].'_50.jpg"/>';
				// ORACION
