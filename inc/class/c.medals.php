...
class tsMedal {
   ...
   */
	public function adGetMedals(){
      ...
      $datos['medallas'] = result_array(db_exec([__FILE__, __LINE__], 'query', 'SELECT u.user_id, u.user_name, m.* FROM w_medallas AS m LEFT JOIN u_miembros AS u ON m.m_autor = u.user_id ORDER BY medal_id DESC LIMIT '.$limit));
		// PAGINAS
		list ($total) = db_exec('fetch_row', db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(*) FROM w_medallas WHERE medal_id > 0'));
   ...
   */
	public function adGetAssign(){
      ...
      $datos['asignaciones'] = result_array(db_exec([__FILE__, __LINE__], 'query', 'SELECT u.user_id, u.user_name, a.*, p.post_id, p.post_title, c.c_nombre, c.c_seo, f.foto_id, f.f_title, w.* FROM w_medallas_assign AS a LEFT JOIN u_miembros AS u ON u.user_id = a.medal_for LEFT JOIN p_posts AS p ON p.post_id = a.medal_for LEFT JOIN p_categorias AS c ON c.cid = p.post_category LEFT JOIN f_fotos AS f ON f.foto_id = a.medal_for LEFT JOIN w_medallas AS w ON w.medal_id = a.medal_id ORDER BY a.medal_date DESC LIMIT '.$limit));
		// PAGINAS
		list ($total) = db_exec('fetch_row', db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(*) FROM w_medallas_assign WHERE id > 0'));
   ...
   */
   private function sameCheck(array $medalla = [], int $mid = 0) {
		...
		if($medalla['type'] >= 1 OR $medalla['type'] <= 3) {
			...
			return db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', "SELECT medal_id FROM `w_medallas` WHERE `m_type` = {$medalla['type']} AND `m_cant` = {$medalla['cant']} $add$mid")) ? false : true;
		}
   }
	/**
    * @name editMedal()
	...
	public function editMedal() {
      ...
      $medalla = self::sameArrayMedal();
		if(is_numeric($medalla['type']) && is_numeric($medalla['cond_user']) && is_numeric($medalla['cond_user_rango']) && is_numeric($medalla['cond_post']) && is_numeric($medalla['cond_foto'])){
			// ACTUALIZAR
        	if(self::sameCheck($medalla, $mid) === true) {
        		$set = $tsCore->getIUP($medalla, 'm_');
				if(db_exec([__FILE__, __LINE__], 'query', "UPDATE w_medallas SET $set WHERE medal_id = $mid")) return true;
   ...
   */
   public function adNewMedal(){
      ...
      $medalla['date'] = time();
		...
		if(is_numeric($medalla['type']) && is_numeric($medalla['cond_user']) && is_numeric($medalla['cond_user_rango']) && is_numeric($medalla['cond_post']) && is_numeric($medalla['cond_foto'])){
			// INSERTAR
        	if(self::sameCheck($medalla) === true) {
				if(insertInto([__FILE__, __LINE__], 'w_medallas', $medalla, 'm_')) return true;
   ...
   */
   public function AsignarMedalla(){
      global $tsUser, $tsCore;
		// DATOS
		$upper = (int)$tsCore->settings['c_upperkey'];
      $medalla = (int)$_POST['mid'];
		$usuario = ($upper === 0) ? strtolower($_POST['m_usuario']) : $_POST['m_usuario'];
		$usuario = $tsCore->setSecure($usuario);
		$post = isset($_POST['pid']) ? (int)$_POST['pid'] : 0;
		$foto = isset($_POST['fid']) ? (int)$_POST['fid'] : 0;
		...
		if($medalla > 0 AND !empty($usuario) OR $post > 0 OR $foto > 0) {
			...
			if(!db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', "SELECT medal_id FROM w_medallas WHERE medal_id = $medalla$yeltipo LIMIT 1"))) return '0: La medalla no puede ser asignada porque no existe o no corresponde a este tipo de asignaci&oacute;n.';
			// VERIFICAMOS LA IP
			$_SERVER['REMOTE_ADDR'] = $tsCore->validarIP();
			$medAss = ['id' => $medalla, 'date' => time(), 'ip' => $_SERVER['REMOTE_ADDR']];
			if(!filter_var($_SERVER['REMOTE_ADDR'], FILTER_VALIDATE_IP)) return '0: Su IP no se pudo validar';
			// Usuario
			if($usuario) {
				$th = ($upper === 0) ? 'LOWER(user_name)' : 'user_name';
				// El usuario existe?
				if(!db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', "SELECT user_id FROM u_miembros WHERE $th = '$usuario' LIMIT 1"))) 
					return '0: El usuario no existe';
				// Ya tiene la medalla?
				if(db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', "SELECT id FROM w_medallas_assign WHERE medal_id = $medalla && medal_for = $user_id LIMIT 1"))) return '0: El usuario ya tiene esa medalla';
				// Asignamos la asignacion de la medalla
				$medAss['for'] = $user_id;
				if(!insertInto([__FILE__, __LINE__], 'w_medallas_assign', $medAss, 'medal_')) return 'Ocurri&oacute; un error al asignar la medalla';
				// Notificamos
				$medMon['user_id'] = $user_id; 
				$medMon['obj_uno'] = $medalla; 
				$medMon['not_type'] = 15; 
				$medMon['not_date'] = time();
				if(!insertInto([__FILE__, __LINE__], 'u_monitor', $medMon)) return 'Ocurri&oacute; un error al notificar al usuario';
				// Debe continuar...
				$continuar = true;
			// Post
			} elseif($post) {
				$query = db_exec([__FILE__, __LINE__], 'query', "SELECT post_id, post_user FROM p_posts WHERE post_id = $post LIMIT 1");
				// El post existe?
				if(!db_exec('num_rows', $query)) return '0: El post no existe';
				$datosdelpost = db_exec('fetch_assoc', $query);
	        	// El post ya tiene medalla?
		    	if(db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', "SELECT id FROM w_medallas_assign WHERE medal_id = $medalla && medal_for = $post LIMIT 1"))) 
		    		return '0: El post ya tiene esa medalla';
				// Asignamos la asignacion de la medalla
				$medAss['for'] = $post;
				if(!insertInto([__FILE__, __LINE__], 'w_medallas_assign', $medAss, 'medal_')) 
					return 'Ocurri&oacute; un error al asignar la medalla';
				// Notificamos
				$medMon['user_id'] = (int)$datosdelpost['post_user']; 
				$medMon['obj_uno'] = $medalla; 
				$medMon['obj_dos'] = $post; 
				$medMon['not_type'] = 16; 
				$medMon['not_date'] = time();
			...
			// Foto
				...
				$query = db_exec([__FILE__, __LINE__], 'query', "SELECT foto_id, f_user FROM f_fotos WHERE foto_id = $foto LIMIT 1");
				// Existe la foto
		    	if(!db_exec('num_rows', $query)) return '0: La foto no existe';
				$datosdelafoto = db_exec('fetch_assoc', $query);
				// La foto ya tiene la medalla?
				if(!db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', "SELECT id FROM w_medallas_assign WHERE medal_id = $medalla && medal_for = $foto LIMIT 1"))) 
				...
				if(!insertInto([__FILE__, __LINE__], 'w_medallas_assign', $medAss, 'medal_')) 
					return 'Ocurri&oacute; un error al asignar la medalla';
				// Notificamos
				$medMon['user_id'] = (int)$datosdelafoto['f_user']; 
				$medMon['obj_uno'] = $medalla; 
				$medMon['obj_dos'] = $foto; 
				$medMon['not_type'] = 17; 
				$medMon['not_date'] = time();
		...
		if($continuar) { 
			return (db_exec([__FILE__, __LINE__], 'query', "UPDATE w_medallas SET m_total = m_total + 1 WHERE medal_id = $medalla")) ? '1: Medalla asignada' : 'La medalla se asign&oacute;, pero hubo un problema y el contador no se alter&oacute;'; 
		} else return 'Hubo problemas, chacho';
 	}
	/**
    * @name delMedalla()
    * @access public
	...
	*/
	public function DelMedalla() {
		...
		if(deleteID([__FILE__, __LINE__], 'w_medallas', "medal_id = $medalla")) {
			if(deleteID([__FILE__, __LINE__], 'w_medallas_assign', "medal_id = $medalla")) {
   ...
   */
	public function DelAssign() {
	   ...
	   if(deleteID([__FILE__, __LINE__], 'w_medallas_assign', "id = $asignacion")) {
	   	// Descontar la asignacion de medalla
		   if(!db_exec([__FILE__, __LINE__], 'query', "UPDATE w_medallas SET m_total = m_total - 1 WHERE medal_id = $medalla")) $msg = '0: Se elimin&oacute; la asignaci&oacute;n, pero no se descont&oacute; de las estad&iiacute;sticas.';
