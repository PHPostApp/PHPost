 ...
 * @author  PHPost Team
 */
include_once TS_TOOLS . "tool.reCaptcha.php";
class tsUser extends reCaptcha {
	...
	*/
	public function logoutUser(int $user_id, $redirectTo = '/'){
		global $tsCore;
		/* BORRAR SESSION */
      $this->session = new tsSession();
      $this->session->read();
	...
	*/
	function userActivate($tsUserID = 0, $tsKey = 0){
		...
		if(db_exec('num_rows', $query) == 0 || $tsKey != $tsKeyLocal){
		    ...
		    if(db_exec([__FILE__, __LINE__], 'query', 'UPDATE u_miembros SET user_activo = 1 WHERE user_id = \''.$tsUserID.'\'')) {
                return $tsData;
			}
			else return false;
		}
    ...
    */
    function com_banned(){
        ...
        //
		$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT *, c_nombre, c_nombre_corto FROM c_baneados LEFT JOIN c_comunidades ON c_id = ban_comunidad WHERE ban_user = \''.$this->uid.'\' LIMIT 1');
        $data = db_exec('fetch_assoc', $query);        
        //
        $now = time();
        //
        if($data['ban_termina'] > 1 && $data['ban_termina'] < $now){
		    db_exec([__FILE__, __LINE__], 'query', 'UPDATE c_miembros SET m_permisos = \'3\' WHERE m_user = \''.$this->uid.'\' AND m_comunidad = \''.$data['ban_comunidad'].'\'');
			db_exec([__FILE__, __LINE__], 'query', 'DELETE FROM c_baneados WHERE ban_id = \''.$data['ban_id'].'\'');
