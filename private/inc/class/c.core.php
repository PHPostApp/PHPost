...
class tsCore {
	...
	public $oauth;		// CONFIGURACIONES DEL SITIO
	...
	public function __construct() {
      ...
      $this->settings['css'] = $this->settings['tema']['t_url'].'/css';
		...
		# Recursos externos
		$this->settings['public'] = $this->settings['url'].'/public/assets';
		//
	...
	*/
	public function getSettings(string $type = 'config') {
		$sql = $type == 'config' ? '* FROM w_configuracion' : ($type == 'extras' ? 'optimizar, extension, width, height, calidad, smarty_cache, smarty_security, smarty_compress, smarty_lifetime FROM w_extras' : 'seo_titulo, seo_descripcion FROM w_site_seo');
		$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT ' . $sql);
