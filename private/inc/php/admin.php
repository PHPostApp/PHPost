...
\*********************************/
	...
	if(file_exists(TS_TOOLS . 'tool.optimizer.php') AND extension_loaded('gd')) {
		$smarty->assign('optimizer', true);
	} else $smarty->assign('optimizer', false);
	// ACTION
	$action = htmlformat($_GET['action']);
	// ACTION 2
	$act = htmlformat($_GET['act']);
	// CLASE POSTS
	$tsAdmin = new tsAdmin();
	...
	// CREAR COPIA DE SEGURIDAD
		...
		require_once TS_TOOLS . 'tool.backup.php';
