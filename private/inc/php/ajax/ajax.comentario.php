...
\*********************************/
	...
	switch($action){
		case 'comentario-preview':
			...
			$smarty->assign("tsComunidadesment",$preview);
            $smarty->assign("tsType",$_GET['type']);
		break;
		case 'comentario-agregar':
			//<--
            if(empty($do)){
    			$tsComunidadesment = $tsPosts->newComentario();
                $smarty->assign("tsType",'new');
                //
    			if(is_array($tsComunidadesment)) $smarty->assign("tsComunidadesment",$tsComunidadesment);
    			else die($tsComunidadesment);
            } elseif($do == 'fotos'){
               //
               $tsComunidadesment = $tsFotos->newComentario();
    			if(is_array($tsComunidadesment)) $smarty->assign("tsComunidadesment",$tsComunidadesment);
    			else die($tsComunidadesment);
                // NUEVA PLANTILLA
		...
		case 'comentario-ajax':
			...
			$tsComentarios = $tsPosts->getComentarios($tsPost);
			$tsComentarios = array('num' => $tsComentarios['num'], 'data' => $tsComentarios['data'], 'block' => $tsComentarios['block'], 'autor' => $tsAutor);
			$smarty->assign("tsComentarios",$tsComentarios);	
